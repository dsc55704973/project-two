...
# Database Setup
#################################################
engine = create_engine("sqlite:///Breweries_wineries.sqlite")
...
# table reference
breweryDB = Base.classes.TABLENAME # input name of table
...
def barNames():
    ...
    # Query bar names
    results = session.query(breweryDB.name).all()
    session.close()
...
def barData():
    ...
    for latitude, longitude, rating, category, address1, zip_code in results:
        ...
        bar_data.append(passenger_dict)
